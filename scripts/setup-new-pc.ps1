# ==============================================================
# SETUP-NEW-PC.ps1 — полная установка рабочего окружения
# Запускать на НОВОМ компьютере (Windows 11)
# ==============================================================
param(
...
# Lock version via icacls (stronger than autoUpdate: false)
...
# cc-start.py — clears env/model before ds mode
@'
...
print('env cleared')
'@ | Out-File -FilePath "$claudeDir\cc-start.py" -Encoding UTF8

# cc-end.py — restores settings
@'
import os, shutil
s = os.path.expandvars(r'%USERPROFILE%\.claude\settings.json')
b = os.path.expandvars(r'%USERPROFILE%\.claude\settings.backup.json')
...
'@ | Out-File -FilePath "$claudeDir\cc-end.py" -Encoding UTF8
...
$settings | ConvertTo-Json -Depth 6 | Out-File -FilePath "$claudeDir\settings.json" -Encoding UTF8
...
# ======================================
# Claude Code + DeepSeek алиасы
# ======================================

# ds-server — запуск прокси (отдельное окно)
...
# quick-launch: старт прокси + ds в одном окне
function dsfull {
    ds-server
...
if (Test-Path $PROFILE) {
    ...
    $profileFunctions | Out-File -FilePath $PROFILE -Encoding UTF8
}
...
Write-Host ""
Write-Host "1. VS Code extensions:" -ForegroundColor $cyan
...
Write-Host ""
Write-Host "2. VS Code settings.json → add:" -ForegroundColor $cyan
...
Write-Host '   ]'
Write-Host ""
Write-Host "3. GoodbyeDPI (если нужен DPI bypass):" -ForegroundColor $cyan
Write-Host "   Скачай: https://github.com/ValdikSS/GoodbyeDPI/releases"
Write-Host ""
Write-Host "=== USAGE ===" -ForegroundColor Yellow
Write-Host ""
Write-Host "  Терминал 1:  ds-server" -ForegroundColor $green
Write-Host "  Терминал 2:  ds" -ForegroundColor $green
Write-Host ""
Write-Host "  Или в одном окне:  dsfull" -ForegroundColor $green
Write-Host ""
